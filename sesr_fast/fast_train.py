...
transform_list = [
    ...
    A.RandomRotate90(p=1.0),  # 90, 180, 270, 360 degreee
    A.RandomCrop(height=256, width=256, p=1.0)  # 256x256 crop
]
train_transform = A.Compose(transform_list)

# 명령행 인자 파서 설정
parser = argparse.ArgumentParser(description='Train QuickSRNet models.')
parser.add_argument('--model', type=str, choices=['m3', 'm5', 'm7', 'm11'], required=True,
...
# DIV2K.pt 데이터셋만 사용할 경우
else:
    train_hr_dir = "/database/hanni/DIV2KS/DIV2K_train_HR"
val_hr_dir = "/database/hanni/DIV2KS/DIV2K_valid_HR"
...
# 데이터셋 및 데이터로더 생성
train_dataset = LoadFastDataset(train_hr_images, args.scaling_factor, transform=train_transform)
val_dataset = LoadFastDataset(val_hr_images, args.scaling_factor, transform=None)
...
# clipped relu 사용할 경우
use_clipped_relu_str = "clipped" if args.clipped_relu else "p"
# concat residual 사용할 경우
...
def train(epoch):
    ...
    for i, (inputs, targets) in enumerate(tqdm(train_loader, desc='Training')):
        ...
        if epoch % args.checkpoint_save == 0:
            ...
            checkpoint_save_path = f'./checkpoint/sesr_{fn_name}_{epoch}.pth'
...
def validate(epoch):
    ...
    if average_psnr > best_psnr:
        ...
        if not os.path.exists('./checkpoint'):
            os.makedirs('./checkpoint')
        checkpoint_save_path = f'./checkpoint/sesr_{fn_name}_best.pth'
        save_checkpoint(epoch, checkpoint_save_path)
...
def main():
    ...
    if os.path.exists(args.checkpoint_load_path):
        ...
        fn_name = f'{use_concat_str}_{use_clipped_relu_str}_{args.model}_{args.scaling_factor}_90_256'
        ...
        wandb.init(project='lx_semicon_sesr_FAST',
                name=f'SESR_{fn_name}',
                entity='ohsy0512',
                config=config_dict)
